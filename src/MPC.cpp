...
Output MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (auto i = 0; i < N-1 ; i++){
  	out.x.push_back(solution.x[x_start+i]);
  	out.y.push_back(solution.x[y_start+i]);
  	out.delta.push_back(solution.x[delta_start+i]);
  	out.a.push_back(solution.x[a_start+i]);
  }
